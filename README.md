...
- 平局视为全部失败，奖池归协议；单边投注正常结算（按赢家数量判断收费方式）；异常情况由管理员暂停市场并取消轮次，投注者全额退款
...
**合约：**
- Registry / Market / Round / Ticket 核心对象
...
- **异常处理**：价格获取失败时交易自动 revert，管理员可暂停市场（同时取消进行中的轮次）
